...
namespace Configurator.WebServer;

// iPad17 User-Agent Header
...
/// <summary>
/// Represents an HTTP server that listens for and handles incoming HTTP requests.
/// </summary>
public class HttpServer
{
    ...
    private void AddFirewallRule(int port)
    {
        ...
        sb.Append($"netsh advfirewall firewall add rule name=AergiaFalconConfigure_Port{port} dir=in action=allow protocol=TCP localport={port}");
//        sb.Append($"&& netsh http add urlacl url=http://+:{port}/ user=Everyone");
        Process process = new Process();
        process.StartInfo.FileName = "cmd.exe";
    ...
    private async Task HandleHttpRequest(HttpListenerContext context)
    {
        ...
        if (path == "/index.html")
        {
            var html = 
                ...
                "<html>\n" +
                "<head>\n" +
                "<title>Aergia Falcon Configure Debug</title>\n" +
                "<meta charset=\"UTF-8\">\n" +
                "<script type=\"module\">\n" +
                $"  import {{ConfigData}} from \"./{_option.IndexFile}\";\n"+
                "   const configure = ConfigData;\n"+
                "</script>\n"+
                "</head>\n"+
                "<body>\n"+
                "</body>\n"+
